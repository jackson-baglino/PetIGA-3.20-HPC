...
PetscErrorCode InitialIceGrains(IGA iga,AppCtx *user)
{
  ...
  if (readFlag==0)
  {
    ...
    for(ii=0;ii<tot*numb_clust;ii++)
    {
      ...
      // Check to the right
      if(xc[0]+rc > user->Lx) flag = 0;

      // Check to the left
      if(xc[0]-rc < 0) flag = 0;

      // Check the top
      if(xc[1]+rc > user->Ly) flag = 0;
...
int main(int argc, char *argv[]) {
  ...
  user.NCice      = 100; //less than 200, otherwise update in user
  user.RCice      = 0.2e-4;
  user.RCice_dev  = 0.5;

  user.readFlag   = 0; // 0 -> generate new ice grains; 1 -> read ice grains from file
  // user.datafile   = "./input/grainReadFile-2.csv";

  //initial conditions
  user.hum0         = 0.98; //initial rel humidity
  user.temp0        = -0.01;
  user.grad_temp0[0] = 0.0/Lx;  user.grad_temp0[1] = 0.0000/Ly;  user.grad_temp0[2] = 0.0/Lz;
  ...
  PetscReal t_final = 300;
  // PetscReal t_final = 21.0*24.0*3600.0;
  //output
